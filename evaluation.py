...
# Get precicion
def getPrecision(mat, k, s, total): #(bins_species, n_pred_labels, n_true_labels, total_binned)
	sum_k = 0
	for i in range(k):
		max_s = 0
		for j in range(s):
	...
	# print("Corrected bined : ",sum_k)
	# print("precision total  : ",total)
	# print("precision, sum_k",sum_k)

	return sum_k/total

# Get recall
def getRecall(mat, k, s, total, unclassified): # (bins_species, n_pred_labels, n_true_labels, total_binned, (ground_truth_count-total_binned))
	sum_s = 0
	for i in range(s):
		max_k = 0
		for j in range(k):
			if mat[j][i] > max_k:
				max_k = mat[j][i]
		sum_s += max_k
	# print("recall, sum_s",sum_s)
	# print("recall total",total)
	# print("recall unclassified",unclassified)
...
def validate_performance(true_labels, pred_labels):
	...
	# print(n_true_labels, n_pred_labels)
	total_binned = 0
	# print("n_true_labels",n_true_labels)
	# print("n_pred_labels",n_pred_labels)
	# print("ReMap",ReMap)
	# print("ReMap_true",ReMap_true)
	bins_species = [[0 for x in range(n_true_labels)] for y in range(n_pred_labels)]
	for i in pred_labels:
		if i in true_labels: # if node exist in the ground truth
			total_binned += 1
			bins_species[ReMap[pred_labels[i]]][ReMap_true[true_labels[i]]] += 1
	# print("bins_species",bins_species)
